...
$testFile =  $($MyInvocation.MyCommand.Definition | Split-Path -Leaf) # This script
Write-Host ("Encrypting: {0}" -f $testFile)
$clearPassword = "0000zZzZzZ-"
$password = ConvertTo-SecureString -String $clearPassword  -Force -AsPlainText 
...
if ($ExecutionContext.SessionState.LanguageMode="FullLanguage") {
    ...
    Get-Childitem "${testFile}.decrypt"  | Get-FileHash -Algorithm MD5  |FT -Property Hash, @{Name='File'; Expression={Split-Path -Leaf $_.Path} }
